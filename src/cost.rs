...
pub fn evaluate_cuts<'a>(cuts: Vec<(HashSet<&'a str>, HashSet<&'a str>, HashSet<&str>)>, dfg: HashMap<(&str, &str), f64>, dfg_minus: HashMap<(&str, &str), f64>, nx_graph: PyGraph, nx_graph_minus: PyGraph, max_flow_graph: HashMap<(&str, &str), f64>, max_flow_graph_minus: HashMap<(&str, &str), f64>, activities_minus: HashSet<&str>, log_variants: HashMap<Vec<&str>, f64>, log_length: f64, log_minus_length: f64, feat_scores: HashMap<(&str, &str), f64>, feat_scores_toggle: HashMap<(&str, &str), f64>, sup: f64, ratio: f64, size_par: f64) -> CutEvaluations<'a> {
    cuts.par_iter().flat_map(|(part_a, part_b, cut_types)| {
        ...
        let (start_part_a, end_part_a, start_part_b, input_part_b, output_part_b) = get_activity_sets(&dfg, &part_a, &part_b);
        let (start_part_a_minus, end_part_a_minus, start_part_b_minus, input_part_b_minus, output_part_b_minus) = get_activity_sets(&dfg_minus, &part_a, &part_b);
...
fn get_start_activities_from_dfg_with_artificial_start<'a>(dfg: &HashMap<(&'a str, &'a str), f64>, activities: &HashSet<&str>) -> HashSet<&'a str> {
    dfg.keys().filter(|(s, t)| *s == "start" && activities.contains(t)).map(|(_, t)| *t).collect()
}

fn get_end_activities_from_dfg_with_artificial_end<'a>(dfg: &HashMap<(&'a str, &'a str), f64>, activities: &HashSet<&str>) -> HashSet<&'a str> {
    dfg.keys().filter(|(s, t)| activities.contains(s) && *t == "end").map(|(s, _)| *s).collect()
}

fn get_input_activities_from_dfg<'a>(dfg: &HashMap<(&'a str, &'a str), f64>, activities: &HashSet<&str>) -> HashSet<&'a str> {
    dfg.keys().filter(|(s, t)| !activities.contains(s) && activities.contains(t)).map(|(_, t)| *t).collect()
}

fn get_output_activities_from_dfg<'a>(dfg: &HashMap<(&'a str, &'a str), f64>, activities: &HashSet<&str>) -> HashSet<&'a str> {
    dfg.keys().filter(|(s, t)| activities.contains(s) && !activities.contains(t)).map(|(s, _)| *s).collect()
}
...
fn fit_loop(log_variants: &HashMap<Vec<&str>, f64>, part_a: &HashSet<&str>, part_b: &HashSet<&str>, end_part_a: &HashSet<&str>, start_part_a: &HashSet<&str>) -> f64 {
    let mut count = 0.0;
...
fn cost_seq(graph: &PyGraph, part_a: &HashSet<&str>, part_b: &HashSet<&str>, start_set: &HashSet<&str>, end_set: &HashSet<&str>, sup: f64, flow: &HashMap<(&str, &str), f64>, scores: &HashMap<(&str, &str), f64>) -> f64 {
    ...
    let part_a_with_start_to_part_b_with_end_num = edge_boundary_directed_num(graph, &part_a_with_start, &part_b_with_end);
...
fn cost_par(graph: &PyGraph, part_a: &HashSet<&str>, part_b: &HashSet<&str>, sup: f64) -> f64 {
    ...
    for &node_a in part_a {
        for &node_b in part_b {
            let c = sup * graph.out_degree(node_a) * graph.out_degree(node_b) / (part_a_out_degree + part_b_out_degree);
...
fn cost_loop(graph: &PyGraph, part_a: &HashSet<&str>, part_b: &HashSet<&str>, start_part_a: &HashSet<&str>, end_part_a: &HashSet<&str>, input_part_b: &HashSet<&str>, output_part_b: &HashSet<&str>, sup: f64) -> Option<f64> {
    ...
    let output_part_b_to_start_part_a_num = edge_boundary_directed_num(graph, output_part_b, start_part_a);
    let end_part_a_to_input_part_b_num = edge_boundary_directed_num(graph, end_part_a, input_part_b);
    let m_p = f64::max(output_part_b_to_start_part_a_num, end_part_a_to_input_part_b_num);

    let cost_1 = node_to_nodes_num(graph, "start", part_b) + nodes_to_node_num(graph, part_b, "end");
    ...
    if !output_part_b.is_empty() {
        for &node_a in start_part_a {
            for &node_b in output_part_b {
                let c = m_p * sup * (node_to_node_num(graph, "start", node_a) / start_to_start_part_a_num) * (node_to_nodes_num(graph, node_b, start_part_a) / output_part_b_to_start_part_a_num);
    ...
    if !input_part_b.is_empty() {
        for &node_a in end_part_a {
            for &node_b in input_part_b {
                let c = m_p * sup * (node_to_node_num(graph, node_a, "end") / end_part_a_to_end_num) * (nodes_to_node_num(graph, end_part_a, node_b) / end_part_a_to_input_part_b_num);
