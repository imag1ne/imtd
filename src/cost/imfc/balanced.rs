...
pub(crate) struct BalancedSequenceCutCost<'a, 'b> {
...
impl<'a, 'b> BalancedSequenceCutCost<'a, 'b> {
    pub(crate) fn new(
        start_nodes_in_part_b: &'a HashSet<&'b str>,
        end_nodes_in_part_a: &'a HashSet<&'b str>,
        sup: f64,
...
impl CutCost for BalancedSequenceCutCost<'_, '_> {
    fn evaluate(&self, graph: &PyGraph, part_a: &HashSet<&str>, part_b: &HashSet<&str>) -> f64 {
        ...
        for &end_node in self.end_nodes_in_part_a {
            for &start_node in self.start_nodes_in_part_b {
                let expected = avg_weight_a_end_to_b_start
        ...
        let deviating_cost = (1.0 - self.sup) * cost_1;
        let missing_cost = self.sup * (cost_2 + cost_3);
        deviating_cost + missing_cost
    }
}

pub(crate) struct BalancedExclusiveChoiceCutCost {
    sup: f64,
}

impl BalancedExclusiveChoiceCutCost {
    pub(crate) fn new(sup: f64) -> Self {
        BalancedExclusiveChoiceCutCost { sup }
    }
}

impl CutCost for BalancedExclusiveChoiceCutCost {
    fn evaluate(&self, graph: &PyGraph, part_a: &HashSet<&str>, part_b: &HashSet<&str>) -> f64 {
        let cost_1 = edge_boundary_directed_num(graph, part_a, part_b);
        let cost_2 = edge_boundary_directed_num(graph, part_b, part_a);
        (1.0 - self.sup) * (cost_1 + cost_2)
    }
}

pub(crate) struct BalancedParallelCutCost {
    sup: f64,
}

impl BalancedParallelCutCost {
    pub(crate) fn new(sup: f64) -> Self {
        BalancedParallelCutCost { sup }
    }
}

impl CutCost for BalancedParallelCutCost {
    fn evaluate(&self, graph: &PyGraph, part_a: &HashSet<&str>, part_b: &HashSet<&str>) -> f64 {
        let mut cost_1 = 0.0;
        let mut cost_2 = 0.0;
        ...
        for &node_a in part_a {
            for &node_b in part_b {
                let expected = graph.out_degree(node_a) * graph.out_degree(node_b)
                    / part_a_and_part_b_out_degree;
                cost_1 += f64::max(
                    0.0,
        ...
        self.sup * (cost_1 + cost_2)
    }
}

pub(crate) struct BalancedLoopCutCost<'a, 'b> {
...
impl<'a, 'b> BalancedLoopCutCost<'a, 'b> {
    pub(crate) fn new(
        start_part_a: &'a HashSet<&'b str>,
        end_part_a: &'a HashSet<&'b str>,
        input_part_b: &'a HashSet<&'b str>,
...
impl CutCost for BalancedLoopCutCost<'_, '_> {
    fn evaluate(&self, graph: &PyGraph, part_a: &HashSet<&str>, part_b: &HashSet<&str>) -> f64 {
        ...
        if !self.output_part_b.is_empty() {
            for &node_a in self.start_part_a {
                for &node_b in self.output_part_b {
                    let expected =
                        max_avg_weight * graph.out_degree(node_a) * graph.out_degree(node_b)
        ...
        let deviating_cost = (1.0 - self.sup) * (cost_1 + cost_2 + cost_3);
        let missing_cost = self.sup * (cost_4 + cost_5);
        deviating_cost + missing_cost
    }
}
