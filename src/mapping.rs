...
use pyo3::{pyfunction, types::{PyAny, PyAnyMethods}, Bound, PyResult};

#[pyfunction]
#[pyo3(signature = (event_log, case_id_key=None, activity_key=None))]
pub fn edge_case_id_mapping<'a>(event_log: Vec<Bound<'a, PyAny>>, case_id_key: Option<&str>, activity_key: Option<&str>) -> PyResult<HashMap<(&'a str, &'a str), HashSet<&'a str>>> {
    ...
    for trace in event_log {
        let case_id = trace.getattr("attributes")?.get_item(case_id_key)?.extract::<&str>()?;
