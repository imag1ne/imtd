...
class IMFParameters(Enum):
    ...
    ORIGINAL_UNDESIRABLE_LOG = "original_undesirable_log"
    ORIGINAL_UNDESIRABLE_DFG = "original_undesirable_dfg"
...
class IMFUVCL(IMF[IMDataStructureUVCL]):
    def apply(self, obj: IMDataStructureUVCL, parameters: Optional[Dict[str, Any]] = None,
              second_iteration: bool = False) -> ProcessTree:
        noise_threshold = exec_utils.get_param_value(IMFParameters.NOISE_THRESHOLD, parameters, 0.0)
        filter_ratio = exec_utils.get_param_value(IMFParameters.FILTER_RATIO, parameters, 0.0)
        original_undesirable_dfg = exec_utils.get_param_value(IMFParameters.ORIGINAL_UNDESIRABLE_DFG, parameters, None)
        ...
        if tree is None:
            ...
            if tree is None:
                if not second_iteration:
                    filtered_ds = self.__filter_dfg_undesirable(obj, original_undesirable_dfg, filter_ratio)
                    filtered_ds = self.__filter_dfg_noise(filtered_ds, noise_threshold)
    ...
    def __filter_dfg_undesirable(self, obj, original_undesirable_dfg, filter_ratio):
        start_activities = copy(obj.dfg.start_activities)
        end_activities = copy(obj.dfg.end_activities)
        dfg = copy(obj.dfg.graph)

        graph = filter_dfg_knapsack(dfg, original_undesirable_dfg, filter_ratio)
