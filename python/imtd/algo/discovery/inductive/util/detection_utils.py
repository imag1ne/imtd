...
def get_connected_components(ingoing, outgoing, activities):
    ...
    for act in ingoing:
        # get all activities that are connected to the current activity
        ingoing_act = set(ingoing[act].keys())
        if act in outgoing:
            ingoing_act = ingoing_act.union(set(outgoing[act].keys()))

        # add the current activity to the set
        ingoing_act.add(act)

        # if the set is not already in the connected components, add it
    ...
    # merge connected components that have common elements
